# Tutorial - Detect threats by using analytics rules in Microsoft Sentinel | Microsoft Learn
...
- An Azure user with the [**Microsoft Sentinel Contributor**](/en-us/azure/role-based-access-control/built-in-roles#microsoft-sentinel-contributor) role assigned on the Log Analytics workspace where Microsoft Sentinel is deployed.
- The following data sources are referenced in this rule. The more of these you have deployed connectors for, the more effective the rule will be. You must have at least one.
    ...
    | **Microsoft Entra ID** | SigninLogsAADNonInteractiveUserSignInLogs |
...
3. From the toolbar at the top of the page, select ![](media/tutorial-log4j-detection/install-update-button.png)**Install/Update**.
...
4. From the filtered list of templates, select **Log4j vulnerability exploit aka Log4Shell IP IOC**. From the details pane, select **Create rule**.
    ...
    [![Screenshot showing how to search for and locate template and create analytics rule.](media/tutorial-log4j-detection/find-template-create-rule.png)](media/tutorial-log4j-detection/find-template-create-rule.png#lightbox)
...
7. Select **Next : Set rule logic**. ![Screenshot of the General tab of the Analytics rule wizard.](media/tutorial-log4j-detection/general-tab.png)
...
3. In the **Alert details** section, let's customize the alert name so that the timestamp of each occurrence appears in the alert title.
    ...
    ![Screenshot of custom details and alert details configurations.](media/tutorial-log4j-detection/custom-details.png)
...
1. Review the remaining settings on the **Set rule logic** tab. There's no need to change anything, though you can if you'd like to change the interval, for example. Just make sure that the lookback period matches the interval in order to maintain continuous coverage.
    ...
    ![Screenshot of remaining rule logic settings for analytics rule.](media/tutorial-log4j-detection/remaining-rule-settings.png)
...
1. Review the settings on the **Incident settings** tab. There's no need to change anything, unless, for example, you have a different system for incident creation and management, in which case you'd want to disable incident creation.
    ...
    ![Screenshot of the Incident settings tab of the Analytics rule wizard.](media/tutorial-log4j-detection/incident-settings-tab.png)
...
1. Select **+ Add new** to create a new automation rule for this analytics rule. This will open the **Create new automation rule** wizard.
    ...
    ![Screenshot of Automated response tab in Analytics rule wizard.](media/tutorial-log4j-detection/add-automation-rule.png)
...
7. Select **Next : Review** to review all the settings for your new analytics rule. When the "Validation passed" message appears, select **Create**. Unless you set the rule to **Disabled** in the **General** tab above, the rule will run immediately.
    ...
    ![Screenshot of the Review and Create tab of the Analytics rule wizard.](media/tutorial-log4j-detection/review-and-create-tab.png)
...
2. Enter the name (or part of the name) of the analytics rule you created in the **Search** bar. (If it doesn't show up, make sure any filters are set to **Select all**.)
3. Mark the check box next to your rule in the list, and select **Delete** from the top banner. (If you don't want to delete it, you can select **Disable** instead.)
